/**
 * Generated by orval v7.21.0 🍺
 * Do not edit manually.
 * Training Platform API
 * REST API for the training, fitness tracking, and nutrition platform.

Errors use a stable JSON body: statusCode, code, message, path, timestamp, requestId.
See the ErrorResponseDto schema. Access tokens are short-lived JWTs sent as Bearer tokens.
Refresh secrets stay in an HttpOnly cookie and are never returned in JSON.
 * OpenAPI spec version: 1.0.0
 */
import type { CreateNutritionFoodDtoBrand } from './createNutritionFoodDtoBrand';
import type { CreateNutritionFoodDtoDescription } from './createNutritionFoodDtoDescription';
import type { CreateNutritionFoodDtoFiberGPer100g } from './createNutritionFoodDtoFiberGPer100g';

export interface CreateNutritionFoodDto {
  /**
   * @minLength 2
   * @maxLength 150
   */
  name: string;
  /**
   * @maxLength 150
   * @nullable
   */
  brand?: CreateNutritionFoodDtoBrand;
  /**
   * @maxLength 1000
   * @nullable
   */
  description?: CreateNutritionFoodDtoDescription;
  /**
   * Kilocalories per 100 grams. Canonical nutritional basis.
   * @minimum 0
   * @maximum 99999.99
   */
  caloriesPer100g: number;
  /**
   * Protein grams per 100 grams. Not required to match 4 kcal/g.
   * @minimum 0
   * @maximum 1000
   */
  proteinGPer100g: number;
  /**
   * Carbohydrate grams per 100 grams.
   * @minimum 0
   * @maximum 1000
   */
  carbohydratesGPer100g: number;
  /**
   * Fat grams per 100 grams. Not required to match 9 kcal/g.
   * @minimum 0
   * @maximum 1000
   */
  fatGPer100g: number;
  /**
   * Fiber grams per 100 grams. Optional; not deducted from carbs.
   * @minimum 0
   * @maximum 1000
   * @nullable
   */
  fiberGPer100g?: CreateNutritionFoodDtoFiberGPer100g;
}
